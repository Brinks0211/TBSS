...
for Cov in GSAA2AxSA2xS;
    ...
    cd ${RandDir}/${indi}
        ...
        do
        tbss_fill ${indi}_tfce_corrp_tstat${i}.nii.gz 0.95 ${StatsDirList[$j]}/mean_FA ${indi}_tfce_corrp_tstat${i}_fill &
        tbss_fill ${indi}_vox_corrp_tstat${i}.nii.gz 0.95 ${StatsDirList[$j]}/mean_FA ${indi}_vox_corrp_tstat${i}_fill 
    done

    echo "-------${StatsDirList[$j]}--------\n-----short cut--${indi}--------"
    cd ${RandDir}/${indi}
        Files=`ls *_fill.nii.gz`
        for file in ${Files}
        do
        echo "---------shortcut ${file}-----------"
